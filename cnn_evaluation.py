...
def train_model(model, data, labels, batch_size=128, learning_rate=0.0005, weight_decay=1e-4, step_size=5, gamma=0.5, epochs=30):
    """
    ...
    data = torch.as_tensor(data, dtype=torch.float32).clone().detach()
...
def test_model(model, data, labels, batch_size=128):
    """
    Test a PyTorch model and calculate performance metrics.

    Parameters:
        model (torch.nn.Module): The trained model to evaluate.
        data (array-like or torch.Tensor): The input data for testing.
...
def k_fold_evaluation(model, data, labels, k_folds=5, batch_size=128):
    """
    Perform k-fold cross-validation on a PyTorch model with extended metrics.
    ...
    for fold in range(k_folds):
        ...
        test_end = test_start + fold_size if fold < k_folds - 1 else len(data)
        ...
        trained_model = train_model(model_copy, data_train, labels_train, batch_size=batch_size)
        results = test_model(trained_model, data_test, labels_test, batch_size=batch_size)
